...
class Benchmark():
    def __init__(self, mem_cap: int = 200_000_000, cpu_cap=25):
        ...
        Args:
            mem_cap (int):  the max byte of memory this program could use. If the script consumes a size of memory larger
                            than this value, it will exit. default = 20,000,000 bytes.
            cpu_cap (int):  the max percentage of cpu this program could use.
                            If the script consumes larger cpu usage
...
def add_zoomed_res(obj: dict, w: int, h: int):
    """
    向obj对象添加一个成员，该成员新的分辨率为 w:h

    Parameters
    ----------
    obj : dict
        可操作的对象，动态对象，因此改动会直接作用于该参数指向的对象。
    w : int
        新成员的分辨率宽度
    h : int
        新成员的分辨率高度
    ...
    base = obj[f'{base_width}x{base_height}']

    k = f'{w}x{h}'
    obj[k] = {}
    obj[k]['c1'] = [int(w / base_width * v) for v in base['c1']]
    obj[k]['c2'] = [int(w / base_width * v) for v in base['c2']]
    obj[k]['c3'] = [int(w / base_width * v) for v in base['c3']]
...
def autoDecom(args: dict):
    ...
    add_zoomed_res(coords, 2560, 1440)
...
def autoForge(args: dict):
    ...
    coords['3840x2160'] = {}
    coords['3840x2160']['c1'] = [v * 2 for v in coords['1920x1080']['c1']]
    coords['3840x2160']['c2'] = [v * 2 for v in coords['1920x1080']['c2']]
    coords['3840x2160']['c3'] = [v * 2 for v in coords['1920x1080']['c3']]
    coords['3840x2160']['c4'] = [v * 2 for v in coords['1920x1080']['c1']]
    ...
    lines = (0, 2, 4)  # 跳行，防止重复点击到空的格子影响观感(循环的次数是一样的) 大件，奇数行小件
    lines = (1, 3, 5)  # 跳行，防止重复点击到空的格子影响观感(循环的次数是一样的)偶数行小件
    for i in lines:
        for j in range(10):
            if not keyboard.is_pressed(args['break']):
                ...
                # 放入目标装备
                mouse.move(x=coords['c4'][0] + coords['c3'][2] * j, y=coords['c3'][1] + coords['c3'][2] * i)
...
if __name__ == '__main__':
    ...
    plan = 'plan_武僧伊娜双手杖安逸BD'

    plan = 'plan_豆角荆棘滚键盘'
    plan = 'plan_武僧伊娜神龙火幻_冲层'

    plan = 'plan_武僧伊娜分身速刷(水)'
    plan = 'plan_s27_dh_扫射不洁多重'
    plan = 'plan_s27_dh_扫射对戒三刀'
