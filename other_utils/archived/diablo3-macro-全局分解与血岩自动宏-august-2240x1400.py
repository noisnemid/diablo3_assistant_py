...
class Macro(object):
    ...
    def autoDecom(self):
        # 自动分解
        pyautogui.click(x=667, y=627, button='left', _pause=False)  # 调出分解界面
        pyautogui.sleep(0.1)
        pyautogui.click(x=215, y=376, button='left', _pause=False)  # 点击分解按钮
        lines = (0, 2, 4, 1, 3, 5)
        for i in lines:
            for j in range(10):
                if not keyboard.is_pressed('esc'):
                    pyautogui.click(x=1600+66*j, y=760+66*i, button='left', _pause=False)
